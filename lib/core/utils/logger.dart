...
void setupLogger({bool test = false, FirebaseCrashlytics? firebaseClient}) {
  if (test) {
    ...
    debug_logger.Logger.level = debug_logger.Level.all;
...
class _DebugLoggerClient implements LoggerClient {
  ...
  void log({
    ...
    switch (level) {
      case LogLevel.debug:
        if (e != null) {
          _logger.d(message, error: e.toString(), stackTrace:  s ?? StackTrace.current);
        } else {
          _logger.d(message);
        }
        break;
      case LogLevel.warning:
        if (e != null) {
          _logger.w(message, error: e.toString(), stackTrace:  s ?? StackTrace.current);
        } else {
          _logger.w(message);
        }
        break;
      case LogLevel.error:
        if (e != null) {
          _logger.e(message, error:  e.toString(), stackTrace:  s ?? StackTrace.current);
        } else {
          _logger.e(message, error:  null, stackTrace:  s ?? StackTrace.current);
        }
        break;
    }
  }
...
class _ReleaseLoggerClient implements LoggerClient {
